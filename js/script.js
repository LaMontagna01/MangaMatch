...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    const allGenres = await getAniListGenres();
    ...
    if (form) {
        form.addEventListener("submit", async (event) => {
            event.preventDefault(); // ✅ corretto: una sola "f"

            const getMultipleValues = (id) =>
                Array.from(document.getElementById(id).selectedOptions).map(opt => opt.value);

            const selectedGenres = getMultipleValues("genre");  // ✅ rinominato per non sovrascrivere
            const formats = getMultipleValues("format");
            const statuses = getMultipleValues("status");
            const minScore = parseInt(document.getElementById("minScore").value) || 0;
            const maxChapters = parseInt(document.getElementById("maxChapters").value) || 10000;

            const resultDiv = document.getElementById("result");
            resultDiv.innerHTML = "Caricamento...";

            const mangaData = await getMangaByPreferences(selectedGenres, formats, statuses, minScore, maxChapters);
        ...
        console.error('Elemento con id "mangaForm" non trovato.');
    }
});
